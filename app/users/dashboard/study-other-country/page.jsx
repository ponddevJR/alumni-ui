...
const Page = () => {
  ...
  const fetchData = async (
    ...
    try {
      const res = await axios.get(
        apiConfig.rmuAPI + "/dashboard/list/study-other-country",
        {
  ...
  return (
    <>
      <TablePage
        header="รายชื่อศิษย์เก่าที่ไปศึกษาต่อที่ต่างประเทศ"
        theads={["มหาวิทยาลัย", "ประเทศ"]}
        fetchData={fetchData}
        totalPage={totalPage}
        total={total}
        exportData={data?.map((d) => ({
          ...
          มหาวิทยาลัย: d?.work_expreriences[0]?.edu_university,
          ประเทศ: d?.work_expreriences[0]?.company_place,
          "ปีที่ศึกษาต่อ(พ.ศ.)": `${d?.work_expreriences[0]?.year_start} - ${
            d?.year_end ? d?.year_end : "ปัจจุบัน"
          }`,
        }))}
        filterBtn={
          <div title="กรอง" className="relative inline-block">
            ...
            >
              ...
              <option
                value={JSON.stringify({ edu_level: "ปริญญาโท" })}
                className="text-sm"
              >
                ปริญญาโท
              </option>
