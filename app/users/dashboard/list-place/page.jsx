...
const Page = () => {
  ...
  const fetchData = async (
    ...
    try {
      const res = await axios.get(
        apiConfig.rmuAPI + "/dashboard/list/alumni-work",
        {
  ...
  const filterByWork = (e) => {
    ...
    setExtraFilter(value); // ✅ เก็บค่า filterWork
    setPage(1); // reset page
  };

  return (
    <TablePage
      header={"รายชื่อและอาชีพในปัจจุบันของศิษย์เก่า"}
      theads={["อาชีพปัจจุบัน", "บริษัท/สถานศึกษา", "จังหวัด/ประเทศ", ,]}
      fetchData={fetchData}
      totalPage={totalPage}
      total={total}
      page={page}
      ...
      exportData={data.map((d) => ({
        ...
        อาชีพปัจจุบัน: d?.work_expreriences[0]?.continued_study
          ? "กำลังศึกษาต่อ"
          : d?.work_expreriences[0]?.job_position || "ว่างงาน",
        "บริษัท/สถานศึกษา": d?.work_expreriences[0]?.continued_study
          ? d?.work_expreriences[0]?.edu_university
          : d?.work_expreriences[0]?.company_name || "ไม่มีข้อมูล",
        "จังหวัด/ประเทศ":
          d?.work_expreriences[0]?.company_place || "ไม่มีข้อมูล",
      }))}
      filterBtn={
      ...
      {loading ? (
        ...
        data.map((d,index) => (
          <tr
            key={uuid()}
            onClick={() => {
              setPrevPath("/users/dashboard/list-place");
          ...
          >
            ...
            <td className="p-2.5 py-3">
              {d?.work_expreriences[0]?.continued_study
                ? "กำลังศึกษาต่อ"
                : d?.work_expreriences[0]?.job_position || "ว่างงาน"}
            </td>
            <td className="p-2.5 py-3 text-start">
