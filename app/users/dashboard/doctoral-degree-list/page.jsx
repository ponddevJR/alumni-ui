...
const Page = () => {
  ...
  const fetchData = async (
    ...
    try {
      const res = await axios.get(
        apiConfig.rmuAPI + "/dashboard/list/master-degree-list",
        {
  ...
  return (
    <>
      <TablePage
        header="รายชื่อศิษย์เก่าที่ศึกษาต่อในระดับปริญญาเอก"
        theads={["สาขาที่ศึกษาต่อ", "มหาวิทยาลัย"]}
        fetchData={fetchData}
        totalPage={totalPage}
        total={total}
        exportData={data.map((d) => ({
          ...
          สาขาที่ศึกษาต่อ: d?.work_expreriences[0]?.edu_dep,
        ...
        {loading ? (
          ...
          data.map((d,index) => (
            ...
            >
              ...
              <td className="p-2.5 py-3">{d?.work_expreriences[0]?.edu_dep}</td>
