...
const Page = () => {
  ...
  }, [selectYearStart, selectYearEnd, page, sort, facultyId, departmentId]);
      ...
      <div className="mt-5  flex flex-col-reverse lg:grid lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white p-5 rounded-lg border border-gray-300 shadow-md shadow-gray-400">
          ...
          <span className="mt-5 flex items-center gap-2 text-sm">
            ...
            />
            <p>
              ศิษย์เก่า{faculty && faculty.label}
              {department && ` สาขาวิชา${department?.label}`}{" "}
              {selectYearStart && `ปีการศึกษา พ.ศ.${selectYearStart}`}{" "}
              {selectYearEnd && !selectYearStart
                ? `สำเร็จการศึกษาปี พ.ศ.${selectYearEnd} `
                : selectYearEnd && ` - พ.ศ. ${selectYearEnd} `}
              ทั้งหมด ({total || 0} คน)
            </p>
          </span>
          ...
          <div className="flex flex-col mt-1.5 w-full h-[420px] overflow-y-auto ">
            {loading ? (
              ...
              alumniList.map((a) => (
                ...
                >
                  <input
                    checked={selectId.includes(a?.alumni_id)}
                    type="checkbox"
                    name=""
                    id=""
                    onChange={() => {
